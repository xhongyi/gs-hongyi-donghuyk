...
long long loadHash(string hash_name) {
	...
	cudaBindTexture(NULL, tex_hash_table, dev_hash_table, INDEX_NUM*sizeof(int));
	cudaBindTexture(NULL, tex_coordinate, dev_coordinate, coord_num*sizeof(int));
...
int loadRef(string ref_name) {
	...
	cudaBindTexture(NULL, tex_ref_string, dev_ref_string, ref_string.size() * sizeof(char));
...
void freeHash(void) {
	free (hash_table);
	free (coordinate);
	cudaUnbindTexture (tex_hash_table);
	cudaUnbindTexture (tex_coordinate);
	cudaFree (dev_hash_table);
	cudaFree (dev_coordinate);
}

void freeRef(void) {
	cudaUnbindTexture (tex_ref_string);
